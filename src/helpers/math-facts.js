window.addEventListener('load', function (e) {
    ...
    function startGame() {
        operation.classList.replace("visible", "hidden");
        calculator.classList.replace("hidden", "visible");
        resultsForm.classList.replace("visible", "hidden");
        let timeleft = 30;
        timer.value = "Go!"
        let gameTimer = setInterval(function () {
            if (timeleft <= 0) {
                clearInterval(gameTimer);
                resultsForm.classList.replace("hidden", "visible");
                calculator.classList.replace("visible", "hidden");
    ...
    function restartGame() {
        operation.classList.replace("hidden", "visible");
        calculator.classList.replace("visible", "hidden");
        resultsForm.classList.replace("visible", "hidden");
        score.value = 0;
